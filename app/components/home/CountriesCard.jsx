...
const CountriesCard = ({country, image, tag, link, index}) => {
  ...
  return (
    <Link href={link}>
      <div className="md:mb-[18px] group">
        <div
          className={` text-white h-[60px] flex  items-center font-rajdhani font-bold text-[20px] leading-[28px] md:text-[30px] md:leading-[32px] px-[10px] ${
            index % 2 === 0 ? "bg-[#FF902E]" : "bg-[#2A37EA]"
          }`}
        >
          <h2>{tag}</h2>
        </div>
        ...
        >
          <div className="flex flex-wrap gap-[10px] mt-[15px]">
            {country.map((country, index) => (
              <p
                key={index}
                className="text-[16px] md:text-[20px] bg-gray-600 px-[10px] py-[5px] font-bold text-white uppercase font-rajdhani leading-[20px] md:leading-[28px]"
              >
                {country}
              </p>
            ))}
